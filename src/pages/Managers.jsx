...
import { ManagerGrid } from "../utils/grids";
import { Header } from "../components";
import { ManagerCrud } from "../utils/data";

const Managers = () => {
    const [data, setData] = useState([])
    const [isEdit, setIsEdit] = useState(false)
    const managerData = new ManagerCrud()
    ...
    const fetchData = async () => {
        const result = await managerData.fetchManagerData();
        ...
        switch (args.requestType) {
            ...
            case 'save':
                ...
                if (args.action == "edit") {
                    ...
                    const updatedRecord = await managerData.editManagerData(args.data);
                    const index = data.findIndex(record => record.manager_id === updatedRecord.result[0].manager_id);
                    ...
                    const insertedRecord = await managerData.addManagerData(args.data);
                    const index = data.findIndex(record => record.manager_id === insertedRecord.result[0].manager_id);
            ...
            case 'delete':
                ...
                const managerId = await args.promise[0].manager_id
                managerData.deleteManagerData(managerId);
                break;
            case 'searching':
                const searchString = args.searchString
                const searchQueryParams = {search: searchString}
                const searchResult = await managerData.searchManagerData(searchQueryParams)
            ...
            case 'filtering':
                ...
                const filterResult = await managerData.filterManagerData(filterQueryParams)
            ...
            case 'sorting':
                ...
                const sortResult = await managerData.SortManagerData(sortQueryParams)
    ...
    return (
        <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
            <Header category="Page" title="Managers" />
            ...
            >
                <ColumnsDirective>
                    {new ManagerGrid().grid().map((item, index) => (
