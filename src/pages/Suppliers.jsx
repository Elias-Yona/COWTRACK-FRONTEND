...
import { SupplierGrid } from "../utils/grids";
import { Header } from "../components";
import { SupplierCrud } from "../utils/supplierCrud";

const Suppliers = () => {
    const [data, setData] = useState([])
    const [isEdit, setIsEdit] = useState(false)
    const supplierData = new SupplierCrud()
    ...
    const fetchData = async () => {
        const result = await supplierData.fetchSupplierData();
        ...
        switch (args.requestType) {
            ...
            case 'save':
                ...
                if (args.action == "edit") {
                    ...
                    const updatedRecord = await supplierData.editSupplierData(args.data);
                    const index = data.findIndex(record => record.supplier_id === updatedRecord.result[0].supplier_id);
                    ...
                    const insertedRecord = await supplierData.addSupplierData(args.data);
                    console.log("insertedRecord  ", insertedRecord)
                    const index = data.findIndex(record => record.supplier_id === insertedRecord.result[0].supplier_id);
            ...
            case 'delete':
                ...
                const supplierId = await args.promise[0].supplier_id
                supplierData.deleteSupplierData(supplierId);
                break;
            case 'searching':
                const searchString = args.searchString
                const searchQueryParams = {search: searchString}
                const searchResult = await supplierData.searchSupplierData(searchQueryParams)
            ...
            case 'filtering':
                ...
                const filterResult = await supplierData.filterSupplierData(filterQueryParams)
            ...
            case 'sorting':
                ...
                const sortResult = await supplierData.SortSupplierData(sortQueryParams)
    ...
    return (
        <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
            ...
            <div>
              ...
              >
                  <ColumnsDirective>
                      {new SupplierGrid().grid().map((item, index) => (
